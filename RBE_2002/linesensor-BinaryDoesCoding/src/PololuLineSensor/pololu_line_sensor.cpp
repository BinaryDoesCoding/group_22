...
// pololu_line_sensor.cpp - Pololu line sensor class method definitions
/**
 ...
 */
PololuLineSensor::PololuLineSensor(unsigned char leftPin, unsigned char rightPin, unsigned int light, unsigned int dark)
    : LineSensor(light, dark)
{
    pins[0] = leftPin;
    pins[1] = rightPin;
}
...
{
    int leftReading = analogRead(pins[0]);
    int rightReading = analogRead(pins[1]);

    /**
     * Positive error should produce positive rotation.
     * Start with right - left.
     * If the robot steers the wrong way, flip the sign later.
     */
    return (float)(rightReading - leftReading);
}